...
public class Lekerdezes3 implements Initializable {
    ...
    private void SQL() {
        try {
            ...
            String SQL = "SELECT menetrend.Beér,jarmu_azonosito.Járatszám From menetrend INNER JOIN " +
                    "jarmu_azonosito on jarmu_azonosito.Jármű_Azonosító=menetrend.Jármű_Azonosító WHERE" +
                    " jarmu_azonosito.Jármű_Azonosító IN(SELECT Jármű_Azonosító FROM menetrend WHERE " +
                    "Megálló_Azonosító IN( SELECT `Megálló_Azonosító` FROM `megallo_azonosito` WHERE `Megálló_Név` = \"Vértó\" ))" +
                    " AND menetrend.Beér IN(SELECT Beér FROM menetrend WHERE Megálló_Azonosító IN( SELECT `Megálló_Azonosító` FROM" +
                    " `megallo_azonosito` WHERE `Megálló_Név` = \"Vértó\" ))";
            ...
            Logger.getLogger(Lekerdezes3.class.getName()).log(Level.SEVERE,null,ex);
        }
    }
